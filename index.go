...
// 最终是要得到 result
func (h *Handler) ContextHandler(ctx context.Context, c iris.Context) *graphql.Result {
	// get query
	...
	if h.rootObjectFn != nil {
		params.RootObject = h.rootObjectFn(ctx, c.Request())
	}
	...
	if h.graphiql {
		...
		_, raw := c.Request().URL.Query()["raw"]
		if !raw && !strings.Contains(acceptHeader, "application/json") && strings.Contains(acceptHeader, "text/html") {
			renderGraphiQL(c.ResponseWriter(), params)
			//return
		}
	}
	...
	if h.playground {
		...
		_, raw := c.Request().URL.Query()["raw"]
		if !raw && !strings.Contains(acceptHeader, "application/json") && strings.Contains(acceptHeader, "text/html") {
			// todo playground 这块功能后续再添加
			renderPlayground(c.ResponseWriter(), c.Request())
			//return
		}
	}
...
func (h *Handler) ServeHTTP(ctx iris.Context) *graphql.Result {
	return h.ContextHandler(ctx.Request().Context(), ctx)
}
